...
describe("invoice reminders", () => {
  ...
  test("blocks a fourth reminder and reminders sent too close together", () => {
    ...
    expect(registerInvoiceReminder(db, { invoiceDocumentId: issued.documentId!, reminderDate: "2026-06-21" }).ok).toBe(true);
    expect(registerInvoiceReminder(db, { invoiceDocumentId: issued.documentId!, reminderDate: "2026-07-01" }).ok).toBe(true);
    const fourth = registerInvoiceReminder(db, { invoiceDocumentId: issued.documentId!, reminderDate: "2026-07-11" });
    expect(fourth.ok).toBe(false);
    expect(fourth.errors[0]).toContain("more than 3 reminder fees");
